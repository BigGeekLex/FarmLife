...
{
    public class RechargerProvidableController : MonoBehaviour, IRechargerProvidable
    {
        [SerializeField] 
        ...
        private void Awake()
        {
            if (_rechargable != null)
            {
                ...
                _rechargable.GetObsOnChange().Subscribe((x) => Changed?.Invoke(_rechargable.MaxBorder, _rechargable.GetValue())).AddTo(this);
            }
        }
        ...
        {
            return _rechargable.GetMaxBorderModField().Value;
        }
    }
}